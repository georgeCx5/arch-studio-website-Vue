...
<template>
  <body class=" flex flex-col items-center text-neo-very-dark-blue font-spartan">
    <nav
      class=" relative flex justify-between TB:justify-start items-center TB:gap-20 w-full max-w-[425px] TB:max-w-[1024px] p-8 TB:px-24 TB:py-14">
      ...
      <div v-show="isNavOpen" class=" absolute right-0 top-full TB:hidden w-full pl-8">
        <div class=" flex flex-col items-start gap-4 w-full px-12 py-10 bg-neo-very-light-grey">
          <button v-for="item in navData" class=" hover:text-neo-dark-grey text-[2rem] leading-[2.5rem] font-bold">
            {{ item }}
          </button>
        </div>
      </div>
      <!-- Nav Desktop -->
      <div class=" hidden TB:flex justify-between w-[313px]">
    ...
    <RouterView />
    <footer
      class=" relative flex flex-col TB:flex-row TB:justify-between items-center gap-8 w-full max-w-[425px] TB:max-w-[1024px] pb-12 TB:pb-0 text-[1.125rem] leading-[1.5625rem] font-bold">
      <div class=" flex justify-center items-center w-[7.5rem] h-[7.5rem] bg-neo-very-dark-blue">
      ...
      <div class=" flex flex-col TB:flex-row TB:justify-between items-center gap-8 TB:w-[313px]">
        <button v-for="item in navData" class=" text-neo-medium-grey hover:text-neo-very-dark-blue">
          {{ item }}
        </button>
      </div>
      <button
        class=" flex justify-center items-center gap-6 w-[252px] h-[72px] bg-neo-very-dark-blue hover:bg-neo-dark-grey">
        <h4 class=" text-white">
          See Our Portfolio</h4>
        <img class=" h-[1.125rem]" :src="arrowWhite" alt="iconArrow">
      </button>
      <!-- Footer Background Color -->
      <div class=" absolute top-0 w-full h-full pt-[3.75rem] TB:pt-0 TB:pr-[9.75rem] -z-10">
        <div class=" w-full h-full bg-neo-very-light-grey"></div>
      </div>
    </footer>
  </body>
</template>