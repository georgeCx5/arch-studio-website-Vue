...
export default {
   ...
   computed: {
      getHoverState() {
         return this.isHovered ? '#DF5656' : '#FFF'
      }
   },
   props: {
      icon: {
...
<template>
   <button @mouseenter="isHovered = true" @mouseleave="isHovered = false">
      <svg xmlns="http://www.w3.org/2000/svg" width="40" :height="data[icon].hSize">
         <path :fill="getHoverState" :d="data[icon].dPath" />
      </svg>
   </button>
</template>